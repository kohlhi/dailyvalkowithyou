...
之後要做的是「特製版」：除了新增任務與系統設定以外，其他自訂功能都關掉。
也就是不開放上傳圖片、不開放編輯身份與階段、不開放編輯事件池，
角色與圖片改成在程式裡預先配好。做的時候記得：
內建圖包（`public/heroes/`）會從「選項之一」變成「唯一來源」，
預設的身份、四個階段的稱號與每階對應的圖都要先在 `store.ts` 的 `defaultIdentity()` 裡配好。
