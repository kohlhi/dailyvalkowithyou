...
function 做鈴聲(): Blob {
  ...
  for (const 起 of [0, 1.9]) {
    組.forEach((f, i) => 一聲(d, 起 + i * 0.22, f, 1.6, 0.62))
    一聲(d, 起 + 0.66, 1046.5, 1.9, 0.5)
  }
  // 正規化到接近滿刻度，手機喇叭本來就小，留太多空間會聽不到
  let 峰 = 0
  for (let i = 0; i < d.length; i++) if (Math.abs(d[i]) > 峰) 峰 = Math.abs(d[i])
  if (峰 > 0) {
    const 倍 = 0.94 / 峰
    for (let i = 0; i < d.length; i++) d[i] *= 倍
  }
  return 打包WAV(d)
}
